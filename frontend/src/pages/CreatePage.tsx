    ...
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      <section className="grid gap-4 lg:grid-cols-3" aria-label="Media source">
        {options.map((option, index) => {
          ...
          return (
            <motion.div key={option.to} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.08, ease: [0.22, 1, 0.36, 1] }}>
              <Link to={option.to} className="create-source-card group flex h-full min-h-64 flex-col rounded-[1.75rem] border p-6 transition duration-200 hover:-translate-y-1 sm:p-7" style={{ backgroundColor: option.featured ? 'var(--color-create-featured)' : 'var(--color-surface-elevated)', borderColor: option.featured ? 'var(--color-brand-500)' : 'var(--color-border)' }}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex h-13 w-13 items-center justify-center rounded-2xl" style={{ backgroundColor: option.featured ? 'var(--color-brand-500)' : 'var(--color-brand-50)', color: option.featured ? 'var(--color-on-brand)' : 'var(--color-brand-600)' }}><Icon className="h-6 w-6" /></div>
