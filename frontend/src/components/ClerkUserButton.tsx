    ...
    <details className="group relative">
      <summary className="flex h-11 w-11 cursor-pointer list-none items-center justify-center rounded-xl outline-none transition hover:bg-[var(--color-nav-hover)] focus-visible:ring-2 focus-visible:ring-[var(--color-brand-500)]" aria-label="Open account menu">
        {user?.imageUrl ? (
          ...
          <span className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold" style={{ backgroundColor: 'var(--color-brand-500)', color: 'var(--color-on-brand)' }}>MT</span>
        )}
      </summary>
      <div className="absolute right-0 z-50 mt-2 w-52 overflow-hidden rounded-2xl border p-2 shadow-2xl" style={{ backgroundColor: 'var(--color-surface-elevated)', borderColor: 'var(--color-border)' }}>
        <Link to="/app/settings" className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition hover:bg-[var(--color-nav-hover)]" style={{ color: 'var(--color-text-primary)' }}>
